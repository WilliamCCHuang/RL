...
def test_correctness(filename="tasks/maze.txt"):
    ...
    for s, a, r, d, t, ns in zip(state, action, reward, done, truncated, next_state):
        ...
        #     print(f'state: {s}')
        #     print(f'action: {a}')
        #     print(f'next_state: {ns}, pred_next_state: {next_state_prediction}')
        #     print(f'reward: {r}, pred_reward: {reward_prediction}')
        #     print(f'done: {d}, pred_done: {done_prediction}')
        #     print(f'truncated: {t}, pred_truncated: {truncated_prediction}')
        #     breakpoint()
...
def write_gif(filename="lava.txt", algorithm=PPO):
    ...
    images[0].save(f"gridworld_{task_name}.gif", save_all=True, append_images=images[1:], loop=0, fps=2)
