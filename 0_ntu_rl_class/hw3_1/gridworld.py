...
class GridWorld:
    ...
    def __init__(
        ...
        self._current_state_coord = None
    ...
    def _read_maze(self, maze_file: str) -> None:
        ...
        self._state_coord_to_idx = {
            state_coord: i for i, state_coord in enumerate(self._state_list)
        }
        self._idx_to_state_coord = {
            i: state_coord for i, state_coord in enumerate(self._state_list)
        }
    ...
    def set_current_state(self, state) -> None:
        ...
        self._current_state_coord = self._idx_to_state_coord[state]
    ...
    def _get_next_state(self, state_coord: tuple, action: int) -> tuple:
        ...
        """
        if action == 0:
            next_state_coord = state_coord[0] - 1, state_coord[1]
        elif action == 1:
            next_state_coord = state_coord[0] + 1, state_coord[1]
        elif action == 2:
            next_state_coord = state_coord[0], state_coord[1] - 1
        elif action == 3:
            next_state_coord = state_coord[0], state_coord[1] + 1
    ...
    def step(self, action: int) -> tuple:
        ...
        # enter bait state can get a position bait reward but will have step penalty afterward.
        if self._is_bait_state(next_state_coord):
            self.bite()
            return next_state, self._bait_reward, False, False
        
        # the logic of the portal state is included in `self._get_next_state()` above
    ...
    def reset(self) -> int:
        ...
        self._current_state = np.random.choice(self._init_states)  # select one valid initial state
        self.close_door()  # close the door if the door is opened
        self.place_bait()  # place the bait back in the bait state if it is bitten
