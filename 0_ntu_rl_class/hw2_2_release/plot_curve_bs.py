...
def run_Q_Learning(grid_world: GridWorld, iter_num: int, bs: float):
    ...
    grid_world.visualize(
        ...
        title="Q_Learning",
        show=False,
        filename=f"Q_Learning_iteration_bz={bs}.png",
    )
    
    wandb.init(
        project="RL HW2 Reward Loss Curve of Q Learning in Different Sample Batch Size",
        name=f'bs={bs}',
        config={
            "bs": bs,
        }
...
if __name__ == '__main__':
    for bs in [2, 4, 8, 16, 32, 64, 128, 256, 512, 2048]:
        grid_world = init_grid_world()
        run_Q_Learning(grid_world, 50000, bs)
