...
@configclass
class CartWithDoublePendulumEnvCfg(DirectRLEnvCfg):
    # env
    decimation = 2
    ...
    # robot
    robot_cfg: ArticulationCfg = CART_WITH_DOUBLE_PENDULUM_CFG.replace(prim_path="/World/envs/env_.*/Robot")
    cart_dof_name = "rail_2_cart"
    pole_dof_name = "cart_2_pole"
    pendulum_dof_name = "pole_2_double"

    # scene
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=4096, env_spacing=5.5, replicate_physics=True)

    # viewer
    viewer: ViewerCfg = ViewerCfg(eye=(1.8, 2.75, 3), lookat=(0, 2.75, 2.8))

    # reset
    max_cart_pos = 2.4  # the cart is reset if it exceeds that position [m]
    ...
    # action scales
    cart_action_scale = 15  # [N]
    pendulum_action_scale = 15  # [Nm]

    rew_scale_alive = 2.0
    rew_scale_terminated = -5.0
    rew_scale_cart_pos = -0.05
    rew_scale_cart_vel = -0.05
    rew_scale_pole_pos = -0.2
    rew_scale_pole_vel = -0.05
    rew_scale_pendulum_pos = -0.2
    rew_scale_pendulum_vel = -0.05


class CartWithDoublePendulumEnv(DirectRLEnv):
    cfg: CartWithDoublePendulumEnvCfg

    def __init__(self, cfg: CartWithDoublePendulumEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _setup_scene(self):
        ...
        # add ground plane
        spawn_ground_plane(prim_path="/World/ground", cfg=GroundPlaneCfg(color=(0.53, 0.81, 0.98)))
        # clone and replicate
    ...
    def _pre_physics_step(self, actions: dict[str, torch.Tensor]) -> None:
        # actions: (num_env, action_space)
        self.actions = actions

    def _apply_action(self) -> None:
        self.robot.set_joint_effort_target(
            self.actions[:, 0].unsqueeze(dim=-1) * self.cfg.cart_action_scale, joint_ids=self._cart_dof_idx  # the shape of action needs to be (num_envs, 1)
        )
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        total_reward = compute_rewards(
            ...
            pole_pos=normalize_angle(self.joint_pos[:, self._pole_dof_idx[0]]),
            pole_vel=self.joint_vel[:, self._pole_dof_idx[0]],
            pendulum_pos=normalize_angle(self.joint_pos[:, self._pendulum_dof_idx[0]]),
            pendulum_vel=self.joint_vel[:, self._pendulum_dof_idx[0]],
...
def compute_rewards(
    ...
    rew_pendulum_pos = rew_scale_pendulum_pos * torch.sum(torch.square(pendulum_pos).unsqueeze(dim=1), dim=-1)
    ...
    total_reward = rew_alive + rew_termination + rew_pole_pos + rew_pendulum_pos + rew_cart_vel + rew_pole_vel + rew_pendulum_vel
