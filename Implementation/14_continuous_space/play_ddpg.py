from tqdm import tqdm
from multiprocessing import Lock
tqdm.set_lock(Lock())

import argparse
import numpy as np
from pathlib import Path

import gym
import torch

from ddpg import DDPG
from atari_env import make_atari_env

ENV_NAME = 'MinitaurBulletEnv-v0'


def main(resume_path):
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)

    exp_dir = Path(resume_path).parent

    env = make_atari_env(ENV_NAME, video=True)
    env = gym.wrappers.RecordVideo(env, video_folder=str(exp_dir))

    model = DDPG(env.observation_space.shape[0], env.action_space.shape[0], update_weight=1e-3)
    model_state_dict = torch.load(args.resume_path, map_location='cpu')['model']
    model.load_state_dict(model_state_dict)

    total_steps = 0
    total_reward = 0.0
    obs = env.reset()
    while True:
        obs_tensor = torch.tensor(obs).unsqueeze(0)  # (1, 28)
        mu = model.actor(obs_tensor)  # (1, 8)
        action = mu.detach().cpu().numpy()[0]  # (8,)
        action = np.clip(action, -1, 1)

        obs, reward, done, trunc, info = env.step(action)

        total_steps += 1
        total_reward += reward

        if done:
            break

    print(f'Steps: {total_steps}, Reward: {total_reward:.2f}')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--resume-path', type=str)
    parser.add_argument('--seed', type=int, default=20)
    args = parser.parse_args()

    main(args.resume_path)